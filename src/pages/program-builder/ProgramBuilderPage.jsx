...
export function ProgramBuilderPage({ user, isTrainer, programs, programWorkouts, workouts, selectedDate, onProgramCreated, onWorkoutCreated }) {
  ...
  useEffect(() => {
    console.log("Programs page load", {
      programs,
      activePrograms: ownActivePrograms,
    });
  }, [ownActivePrograms, programs]);
  ...
  async function startProgram(event) {
    ...
    const activeProgram = {
      ...
      currentWeek: 1,
      nextWorkoutIndex: 0,
      ...(scheduled ? { workoutDates: buildWorkoutDatesForProgram(nextProgramWorkouts, programStartDate) } : {}),
    };
  ...
  async function assignProgram(event) {
    ...
    await saveUserActiveProgram(assignAthleteId, {
      ...
      currentWeek: 1,
      nextWorkoutIndex: 0,
      ...(scheduled ? { workoutDates: buildWorkoutDatesForProgram(nextProgramWorkouts, assignStartDate) } : {}),
    });
          ...
          <div className="program-card-grid">
            {visiblePrograms.map((program) => {
              ...
              const ownProgressWorkouts = ownActiveProgram?.scheduled
                ...
                : nextProgramWorkouts;
              const ownSummary = ownActiveProgram ? progressSummary(ownProgressWorkouts, workouts) : null;
              const activeSummary = ownSummary || summary;
              ...
              return (
                <article className={`program-card ${expanded ? "expanded" : ""}`} key={program.id}>
                  ...
                  <p className={`program-status program-status-${isProgramActive ? "active" : "idle"}`}>{programStatusLabel(isProgramActive ? "active" : "")}</p>
                  {isProgramActive ? (
                    <>
                      <div className="progress-meter" aria-label={`${activeSummary.percent}% complete`}>
                        <span style={{ width: `${activeSummary.percent}%` }} />
                      </div>
                      <dl className="program-stats">
                        <div>
                          <dt>Complete</dt>
                          <dd>{activeSummary.completed}/{activeSummary.total}</dd>
                        </div>
                        <div>
                          <dt>Next</dt>
                          <dd>{activeSummary.nextDate ? formatDate(activeSummary.nextDate) : "All caught up"}</dd>
                        </div>
                      </dl>
                    </>
                  ) : (
                    <dl className="program-stats program-library-stats">
                      <div>
                        <dt>Length</dt>
                        <dd>{programLengthLabel(nextProgramWorkouts)}</dd>
                      </div>
                    </dl>
                  )}
